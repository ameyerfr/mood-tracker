module.exports = {
  first_time : [
    "Hi, my name is ${this.name} ! Nice to meet you !",
    "Hi, ${this.user} ! I am ${this.name}"
  ],
  cheer_up : [
    "Cheer up! It's going to be better tommorrow.",
    "Everyday is unique, enjoy it the best you can!",
    "I'm sorry you're feeling down today. Virtual hug!",
    "Remember... What doesn't kill you makes you stronger.",
    "Don't worry, everything's going to be alright.",
    "Be positive. Stay happy and don’t let the negativity of the world get you down.",
    "You’re braver than you believe, stronger than you seem, and smarter than you think.",
    "When things go wrong and you feel like giving up – remember you’ve got me!",
    "If at first you don’t succeed, think how many people you’ve made happy.",
    "I know you, you are awesome, you are strong and you will be able to make things better.",
    "You’ll never find a rainbow if you’re looking down."
  ],
  greeting : [
    "Nice to see you again ${this.user}!",
    "I just woke up !",
    "Hi there ! How are you today ?",
    "Hi ${this.user}, I missed you !",
    "Hello ! Did you miss me ?",
    "Today is a brand new day !"
  ],
  thanks : [
    "Thank you very much, this is great !",
    "Nice ! A million thanks to you !",
    "You don't know how much you make me happy. Thanks !",
    "Thanks a lot !",
    "Thank you ${this.user} !"
  ]
}
